...
// 7 Types of Crow's Foot Icons matching ERD-Cloud layout exactly (Spread out towards table end)
const ERD_REL_TOOLS: {
  tool: ActiveTool;
  title: string;
  svg: React.ReactNode;
}[] = [
  {
    ...
    svg: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        ...
        <circle cx="5" cy="12" r="2.5" strokeWidth="1.6" fill="#090d16" />
  ...
  },
  {
    ...
    svg: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
        <line x1="1" y1="12" x2="14" y2="12" strokeWidth="1.6" />
        <circle cx="7" cy="12" r="2.8" strokeWidth="1.6" fill="#090d16" />
        <path d="M 14 12 L 23 5 M 14 12 L 23 19 M 14 12 L 23 12" strokeWidth="1.6" strokeLinecap="round" />
      </svg>
    ),
  },
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-11 bg-[#090d16] border-r border-white/[0.08] flex flex-col items-center py-2 gap-1 z-30 select-none overflow-y-auto">
      {/* 1. Select Tool */}
      <button
      ...
      <button
        onClick={onAddTable}
        className="w-8 h-8 flex items-center justify-center rounded-md text-slate-400 hover:text-white hover:bg-white/[0.06] active:scale-95 transition-all"
        title="테이블 추가 (T)"
      >
      ...
      {/* 4~10: All 7 Crow's Foot Relationship Tools (ERD-Cloud Style Direct List) */}
      ...
      <div className="flex flex-col items-center gap-1 mt-auto pt-2">
        <button
          onClick={onZoomIn}
          className="w-7 h-7 flex items-center justify-center rounded text-slate-400 hover:text-white hover:bg-white/[0.06]"
          title="확대 (+)"
        >
